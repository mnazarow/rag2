...
# Всё, что трогает индекс, запускается через runjob.py, а не напрямую.
# Это не стилистика. Прямой вызов `index.py update` из cron — отдельный
# процесс, который не знает ни о каких блокировках: ночной проход и
# нажатая в админке «Переиндексация» пишут в один и тот же файл векторов,
# каждый из своей копии в памяти, и побеждает сохранивший последним.
# Часть векторов при этом пропадает, а ошибки нет — индекс выглядит
# целым. Через очередь второй запуск получает понятный отказ и пишет о
# нём в журнал.
TASKS = {
    "backup": {
        "title": "резервная копия индекса",
        "cron": config.BACKUP_SCHEDULE,
        "command": "runjob.py backup",
