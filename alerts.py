...
def collect() -> list[dict]:
    ...
    # Индекс, собранный старым конвейером, ищет хуже нового кода — и
    # делает это молча: ни ошибки, ни падения, только просевшее качество.
    try:
        import index as index_mod
        st = index_mod.pipeline_state()
        if st["stale"]:
            add("pipeline_stale", "warning",
                "Индекс собран старой версией конвейера",
                "Разбиение или нормализация текста изменились после последней "
                "полной сборки: индекс и поисковые запросы живут по разным "
                "правилам.",
                "Раздел «База знаний» → полная переиндексация "
                "(python index.py build --force).")
    except Exception:  # noqa: BLE001
        pass
